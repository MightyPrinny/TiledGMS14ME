    ...
    : mSettings(new QSettings(this))
{
    ...
    mQuadColor = colorValue("QuadColor", Qt::white);
    mGridFine = intValue("GridFine", 4);
    mSnapGridHeigth = intValue("SnapGridHeigth",16);
    mSnapGridWidth = intValue("SnapGridWidth",16);
...
void Preferences::setShowQuads(bool showQuads)
{
    if (mShowQuads == showQuads)
        return;

    mShowQuads = showQuads;
    mSettings->setValue(QLatin1String("Interface/ShowQuads"), mShowQuads);
    emit showQuadsChanged(mShowQuads);
}

void Preferences::setSnapGrid(){

    bool ok ;
    int numb = QInputDialog::getInt(nullptr,QLatin1String("Snap Width"),QLatin1String("Width"),mSnapGridWidth,1,1024,1,&ok);
    if(ok)
        mSnapGridWidth = numb;
    numb = QInputDialog::getInt(nullptr,QLatin1String("Snap Heigth"),QLatin1String("Heigth"),mSnapGridHeigth,1,1024,1,&ok);
    if(ok)
        mSnapGridHeigth = numb;

}

QSize Preferences::snapGrid(){

    return QSize(mSnapGridWidth,mSnapGridHeigth);
...
void Preferences::browseExecutablePath()
{
    mExecutablePath=QFileDialog::getOpenFileName(nullptr,
                                                 QLatin1String("Open File"), QLatin1String(""), QLatin1String("Windows Executable (*.exe)"));
...
{
    if (mQuadColor == quadColor)
        return;

    mQuadColor = quadColor;
    mSettings->setValue(QLatin1String("Interface/QuadColor"), mQuadColor.name());
    emit quadColorChanged(mQuadColor);
}
