...
SnapHelper::SnapHelper(MapRenderer *renderer,
...
void SnapHelper::snap(QPointF &pixelPos) const
{
    ...
    if (mSnapToFineGrid || mSnapToGrid) {
        if(mRenderer->isOrthogonal())
        {
            QSize gridSize = Preferences::instance()->snapGrid();
            tileCoords = QPointF(floor(pixelPos.x()/gridSize.width()),floor(pixelPos.y()/gridSize.height()));
            pixelPos = QPointF(tileCoords.x()*gridSize.width(),tileCoords.y()*gridSize.height());
            return;
        }
        else
        {
            tileCoords= mRenderer->pixelToTileCoords(pixelPos);
        }
        if (mSnapToFineGrid) {
