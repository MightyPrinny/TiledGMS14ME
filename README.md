...
-Option to run the level (requires an executable of Megamix Engine to be configured in preferences)
...
-The gmx exporter now adds to the room the bgQuad image that matches the quad size of the map
-The gmx exporter now uses the background color of the map
-Objects have an offset when being moved that can be changed with the offsetX and offsetY custom properties

To generate templates you must select a folder with a copy of the objects and sprites folder from the game maker project
the templates will be generated in that folder inside a folder called templates
